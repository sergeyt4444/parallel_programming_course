...
void RandomizeArray(double* rarray, const long& size, const long& min = -50, const long& max = 50) {
	for (long i = 0; i < size; i++) {
		long part1 = std::rand();
		long part2 = part1 << (sizeof(int) );
		part2 |= std::rand();
		*(rarray + i) = part2 % static_cast<long>(max - min + 1) + min;
	}
}


void PrintPoints(double* X, double* Y, const long& Size) {
	for (long i = 0; i < Size; i++) {
...
void ReinitEqPoints(double* X, double* Y, const long& Size, const long& min = -50, const long& max = 50) {
...
long FindBLPoint(double* X, double* Y, const long& Size) {
	if (Size < 1) { return -1; }
	long pointInd = 0;
...
long FindBLPointParallel(double* X, double* Y, const long& Size) {
	if (Size < 1) { return -1; }
	long pointInd = 0;
	long i;
	int threads = omp_get_max_threads();
	long* pointIndOwn = new long[threads];
...
long FindTRPoint(double* X, double* Y, const long& Size) {
	if (Size < 1) { return -1; }
	long pointInd = 0;
	for (long i = 1; i < Size; i++) {
		if (Y[i] > Y[pointInd]) {
...
long FindPWithMinAngle(double* X, double* Y, const long& Size,
...
long FindPWithMinAngleParallel(double* X, double* Y, const long& Size,
	const double &x1, const double& y1, const double& x2, const double& y2) {
	double maxCos = -1.5;
	long NextPointInd = -1;
	long i;
	double tmp;
	int threads = omp_get_max_threads();
	long* NextPointIndOwn = new long[threads];
...
void ElimPointsOnLines(double* X, double* Y, long* Envelope, long* Size) {
	long i = 1;
	while (i != *Size) {
...
//	PrintPoints(X_coord, Y_coord, Size);
	...
	if (Size == 1) {
		...
		if (Size == 2) {
			...
			long dynsize = step;
			long* Envelope = static_cast<long*>(malloc(sizeof(long) * dynsize));
			long PNum = 1;
			time_part = omp_get_wtime();
			long FirstPoint = FindBLPointParallel(X_coord, Y_coord, Size);
			...
			while (Envelope[PNum] != FirstPoint && ((X_coord[FirstPoint] != X_coord[Envelope[PNum]])
				...
				if (PNum == dynsize) {
					dynsize += step;
					Envelope = static_cast<long*>(realloc(Envelope, sizeof(long) * (dynsize)));
				}
			...
			long* EnvelopeForCheck = static_cast<long*>(malloc(sizeof(long) * dynsize));
			long PNumForCheck = 1;
			int Correct = 1;
			time_part_seq = omp_get_wtime();
			long FirstPointForCheck = FindBLPoint(X_coord, Y_coord, Size);
			...
			while (EnvelopeForCheck[PNumForCheck] != FirstPointForCheck && Correct &&
				...
				if (PNumForCheck == dynsize) {
					dynsize += step;
					EnvelopeForCheck = static_cast<long*>(realloc(EnvelopeForCheck, sizeof(long) * (dynsize)));
				}
